package lib.shoot
{
	...
	import lib.shoot.entity.states.PlayerStateClimb;
	...
	public class Physics
	{
		...
		public static function entityUpdatePhysics(entity:Entity, levelCollisions:Array, entities:Array):void
		{
			if(!CameraManager.getCameraRect().intersects(entity.getBoundingBox().getContainerRect()))
			{
				return;
			}
			...
			var feetCheck:Quadrilateral = new Quadrilateral().fromPositionAndSize(entity.posX, entity.posY + entity.getBoundingBox().height, entity.getBoundingBox().width, 1);
			...
			if(entity.physicsCheckEntities)
			{
				...
				{
					// Checks if the entity is actually on the screen
					var inCameraView:Boolean = CameraManager.getCameraRect().containsRect(entity2.getBoundingBox().getContainerRect());
					...
					if(inCameraView && entity != entity2 && getQualifiedClassName(entity) != getQualifiedClassName(entity2) && entity2.physicsCheckEntities && lineSegmentTemp.setPoints(entity.getBoundingBox().getCenterPoint(), entity2.getBoundingBox().getCenterPoint()).getDistance() < 32)
					{
			...
			if(entity.physicsCheckTiles)
			{
				...
				{
					inCameraView = CameraManager.getCameraRect().containsRect(quad.getContainerRect());
					
					if(inCameraView)
					{
						if(Physics.doShapesCollide(entityPathCollider, quad) || Physics.doShapesCollide(entity.getBoundingBox(), quad) || Physics.doShapesCollide(entityPathColliderInverted, quad))
						{
							var quadrant:String = Physics.getQuadrant(entity, quad);
							
							entity.onTileHit(quadrant);
							...
							{
								entity.posX = quad.leftLine.getMinimum().x - entity.getBoundingBox().width;
								entity.xVel = 0; 
								entity.setState(new PlayerStateClimb(), "LEFT");
							}
							else if(quadrant == "RIGHT" && entity.xVel < 0 && quad.rightLineEnabled)
							{
								entity.setState(new PlayerStateClimb(), "RIGHT");
